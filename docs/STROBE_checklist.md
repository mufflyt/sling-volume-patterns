...
| 8 | For each variable of interest, give sources of data and details of methods of assessment. Describe comparability of assessment methods if there is more than one group | **Specialty Classification:** annual CMS rendering-provider type, the subspecialty certification registry (including the subspecialty certification start-date field for certification year), urology-pathway certification roster; linkage by exact NPI, then by full name and practice state where necessary. Denominator from CMS Program Statistics, table MDCR ENROLL AB 11. | |
